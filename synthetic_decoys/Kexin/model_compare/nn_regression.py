...
def main():
  ...
  nn_cv = GridSearchCV(estimator=model, param_grid=grid, cv=5, n_jobs=1,
                     return_train_score=True, scoring='neg_mean_absolute_error', verbose = 2)
  grid_result = nn_cv.fit(X_train, y_train)

  logfile = DIR_PATH+'model_compare/log/nnreg_'+rna+'.txt'
  ...
  # PREDICT
  pred = best_model.predict(X_test).flatten().tolist()
  test_result['pred'] = pred
  with open(logfile, 'a') as f:
    print("R^2 is: %f" % best_model.score(X_test, y_test), file = f)
  test_result.to_csv(DIR_PATH + 'model_compare/pred/nnreg_'+rna+'.txt', sep = ' ', index = False)
