...
def main():
  ...
  # IMPLEMENT MODEL
  ert = ExtraTreesRegressor(oob_score = True, verbose = 2)
  ...
  ert_cv = GridSearchCV(ert, grid, cv = 5, verbose = 2, scoring = 'neg_mean_absolute_error')
  grid_result = ert_cv.fit(X_train, y_train)

  logfile = DIR_PATH+'model_compare/log/ertreg_'+rna+'.txt'
  ...
  test_result.to_csv(DIR_PATH + 'model_compare/pred/ertreg_'+rna+'.txt', sep = ' ', index = False)
