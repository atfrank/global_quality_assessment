...
def main():
  ...
  rf_cv = GridSearchCV(rf, grid, cv = 5, verbose = 2, scoring = 'neg_mean_absolute_error')
  grid_result = rf_cv.fit(X_train, y_train)

  logfile = DIR_PATH+'model_compare/log/rfreg_'+rna+'.txt'
  ...
  test_result.to_csv(DIR_PATH + 'model_compare/pred/rfreg_'+rna+'.txt', sep = ' ', index = False)
