...
then
    ...
    for rna in ${rnas}
    do
        rm -f rmsd_files/${rna}.txt 
        for i in ${ndecoys}
        do
            rms.pl -fit -fitsel heavy pdb_files/${rna}.pdb decoy_files/${rna}_nlb_decoy_${i}.pdb | awk -v id=${rna} -v model=${i} '{print id, model, $1}' | tee -a rmsd_files/${rna}.txt
        done
    done
