...
# script to gather all corrected chemical shifts
...
rm -f chemical_shift_files/all_measured_corrected.txt

for rna in ${rnas}
do
    awk -v id=${rna} '{print id, $0}' ../reference_errors/${rna}/chemical_shifts_corrected_larmord.txt | tee -a chemical_shift_files/all_measured_corrected.txt
done
