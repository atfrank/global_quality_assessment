...
def main():
  DIR_PATH = '/home/kexin/projects/global_quality_assessment/RF_classifier/'  
  ...
  param_grid = dict(n_estimators = n_estimators, min_samples_split = min_samples_split, bootstrap = bootstrap, max_features = max_features, class_weight = class_weight)
  clf = GridSearchCV(estimator=rf, param_grid=param_grid, cv=5, n_jobs=8, 
  ...
  logfile = DIR_PATH+'log/rf_log_'+rna+'.txt'
  ...
  train_result.to_csv(DIR_PATH + 'pred/rf_train_pred_'+rna+'.txt', sep = ' ', index = False)
  test_result.to_csv(DIR_PATH + 'pred/rf_test_pred_'+rna+'.txt', sep = ' ', index = False)
  
  # save best model
  best_model = clf.best_estimator_
  modelname = DIR_PATH + 'models/rf_model_'+rna+'.sav'
