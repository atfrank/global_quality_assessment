...
source ~/.bashrc
cd /Users/atfrank/GitSoftware/chemical_shift_referencing_errors
...
for pdb in ${pdbs}
do
    cat ${pdb}/detected_errors_larmord.txt | awk -v method=larmord -v id=$pdb '{print method, id, $0}'
    cat ${pdb}/detected_errors_ramsey.txt | awk -v method=ramsey -v id=$pdb '{print method, id, $0}'
done
